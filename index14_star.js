...
document.body.append(listContainer); // Добавляю список в документ

popularDirectors.forEach((item, index) => {
  const listItem = document.createElement("li");
  listItem.innerHTML = `${index + 1}.
  <span class="name">${item.name}</span><br>
  <span class="career">${item.career}</span><br>
    <a href="${item.films}" target="_blank">фильмография</a>`;
  listContainer.append(listItem);
});
container.appendChild(listContainer);

const bestFilmsList = document.getElementById("bestFilmsList");
const topFilmsList = popularDirectors.map(function (popularDirectors) {
  return popularDirectors.top_rated_film;
});
function filmsList() {
  bestFilmsList.textContent = topFilmsList;
}
filmsList();
